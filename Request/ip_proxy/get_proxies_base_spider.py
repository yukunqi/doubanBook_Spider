...
from ip_spider.tools import format_put_data
from ip_spider.data_save import pipeline
from ip_spider.html_parser import parser
from ip_spider.page_downloader import aispider
from ip_spider.threads import start, work_queue, save_queue
from ip_spider.log_format import logger
from ip_proxy.proxy_basic_config import url_parse_dict, target_urls,collection_name
...
class SpiderMain(object):
    ...
    def get_and_check(self, response):
        ...
        html_content=bytes.decode(html_content)  #把bytes转成str
        # 网站名
    ...
    def start_check(self, ips, website_name):
        if ips:
            ...
            for _ip in ips:
                ...
                # 调用format_put_data 构造放入队列中的数据
                put_data = format_put_data(args={"url": url, '_ip': _ip, 'time_out': 5}, work_func=valid,
                                           need_save=True,
                                           save_func=self.save_ip,
                                           meta={'website_name': website_name, 'target_url': url})
                # 放入队列
                work_queue.put(put_data)
        else:
